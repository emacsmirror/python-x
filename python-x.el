...
(defun python-x-setup ()
  "Setup an ESS-like keyboard map in python-mode and python-ts-mode"
  (dolist (mode-map-sym '(python-mode-map python-ts-mode-map))
    (when (boundp mode-map-sym)
      (let ((mode-map (symbol-value mode-map-sym)))
  ...
  (when (featurep 'expand-region)
    (dolist (mode '(python-mode python-ts-mode))
      (er/enable-mode-expansions mode 'python-x-mode-expansions))))
